...
//   · 공유 코드(../core/*, ../*-common/*, ../perception-error/*) → network-first
//   · 앱 고유 정적파일(html·매니페스트·아이콘) → cache-first
//   · 네트워크·캐시 모두 실패 → 흰 화면 대신 반드시 Response 반환.
const APP = 'time-reproduction-adults';
...
const PRECACHE = [
  ...
  '../core/engine.js',
  '../core/i18n.js',
  '../perception-error/score.js',
  '../time-reproduction-common/time-reproduction.js',
];

const isShared = (url) =>
  url.pathname.includes('/core/') || url.pathname.includes('-common/') || url.pathname.includes('/perception-error/');
