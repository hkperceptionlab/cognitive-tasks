...
//   · 공유 코드(../core/*) → network-first
//   · 앱 고유 정적파일(html·매니페스트·아이콘·inattentional-blindness.js) → cache-first
//   · 네트워크·캐시 모두 실패 → 흰 화면 대신 반드시 Response 반환.
// 이 앱은 엔진(runTask)을 쓰지 않는 독립 데모라 core/i18n.js 만 공유한다(core/engine.js 없음).
// inattentional-blindness.js 는 앱 로컬이라 cache-first — 로직을 고치면 캐시 버전(:v2)을 올려야 한다.
const APP = 'inattentional-blindness';
