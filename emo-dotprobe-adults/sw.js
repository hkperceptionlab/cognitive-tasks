...
//
// 전략:
//   · 공유 코드(../core/*, ../*-common/* = 엔진·문구·단어)  → network-first
//       (이 앱은 emo-dotprobe-common 과 emo-stroop-common[단어 재사용] 둘 다 씀 — 둘 다 -common/ 이라 network-first)
//   · 앱 고유 정적파일(html·매니페스트·아이콘)             → cache-first (빠르게 열림)
//   · 네트워크·캐시 모두 실패 → 흰 화면 대신 반드시 Response 반환.
const APP = 'emo-dotprobe-adults';
...
const PRECACHE = [
  ...
  '../core/engine.js',
  '../core/i18n.js',
  '../emo-dotprobe-common/emo-dotprobe.js',
  '../emo-stroop-common/emo-stroop.js',
];

// 공유 코드(엔진·문구·단어)인가 → network-first 대상
